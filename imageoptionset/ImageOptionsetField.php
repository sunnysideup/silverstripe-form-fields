...
class ImageOptionsetField extends OptionsetField
{
    ...
    public static function set_number_per_row($v)
    {
        self::$number_per_row = $v;
    }
    public static function get_number_per_row()
    {
        return self::$number_per_row;
    }
    ...
    public function __construct($name, $title = '', $value = '', $objects, $imageFieldName = "Image", $width = 32, $height = 16)
    {
        $array = array();
        if ($objects) {
        ...
        if ($this->objects) {
            foreach ($this->objects as $obj) {
                ...
                if (self::$number_per_row > 1) {
                    if (!(($count) % self::$number_per_row)) {
                        $firstInRow = " firstInRow";
                    }
                    if (!(($count + 1) % self::$number_per_row)) {
                        $lastInRow = " lastInRow";
                    }
                }
