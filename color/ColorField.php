...
class ColorField extends TextField
{
    ...
    public function __construct($name, $title = null, $value = '#000000')
    {
...
class ColorField_ReadOnly extends ReadonlyField
{
    ...
    {
        if ($this->value) {
            $value = $this->dontEscape ? ($this->reserveNL ? Convert::raw2xml($this->value) : $this->value) : Convert::raw2xml($this->value);
        } else {
            $value = '<i>(' . _t('FormField.NONE', 'none') . ')</i>';
        }
        ...
        if ($this->value) {
            $attributes['style'] = "background-color: $this->value";
        }
